...
class ZonedDateTimeSchema extends Schema<Temporal.ZonedDateTime> {
    ...
    min(value: Temporal.ZonedDateTime): ZonedDateTimeSchema {
        if (boundsContradict(this._checks, TAG_MAX, value, Temporal.ZonedDateTime.compare, 1)) {
    ...
    max(value: Temporal.ZonedDateTime): ZonedDateTimeSchema {
        if (boundsContradict(this._checks, TAG_MIN, value, Temporal.ZonedDateTime.compare, -1)) {
            throw new Error('Minimum must not exceed maximum.');
        }
