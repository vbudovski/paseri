...
<Playground
    schemaDefaultValue={"p.string().min(3)"}
...
A valid [URL](https://url.spec.whatwg.org/).
...
A valid [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) time string `hh:mm:ss[.s+]`, with an optional `Z` suffix.
...
```typescript
p.string().time({ offset: true });
// 01:02:03 ✅
// 01:02:03Z ✅
// 01:02:03+02:30 ✅
```

A timezone designator can be required by setting the `local` option to `false`.

```typescript
p.string().time({ local: false });
// 01:02:03Z ✅
// 01:02:03 ❌
```
