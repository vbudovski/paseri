...
- Run with `deno test -P --unstable-unsafe-proto`. The `-P` flag applies the permissions configured in `deno.json`; without it the suite fails on permission prompts. `--unstable-unsafe-proto` restores `Object.prototype.__proto__` (Deno deletes it by default), so tests exercise the Annex B semantics that browsers and default Node.js give Paseri's users — `__proto__`-key handling bugs are invisible without it.
